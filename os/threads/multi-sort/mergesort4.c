...
void msort(int *arr, int n) {
  ...
  int half_size = n / 2;
  msort(arr, half_size);
  msort(arr + half_size, n - half_size);
  merge(arr, n, half_size);
}
...
//
// Four Threads:
// [pid 95397] real: 1.027s user: 3.315s sys: 0.052
int main () {
  int n = 1 << 24;
  ...
  int h1 = n / 2, // first half items
    h2 = n - h1, // second half items
    q1 = h1 / 2, // first quarter items
    q2 = h1 - q1, // second quarter items
    q3 = h2 / 2, // third quarter items
    q4 = h2 - q3; // fourth quarter items
  
  pthread_t p1, p2, p3, p4;
  ThreadArgs
    args1 = {arr, q1}, // 1st quarter
    args2 = {arr + q1, q2}, // 2nd quarter
    args3 = {arr + q1 + q2, q3}, // 3rd quarter
    args4 = {arr + q1 + q2 + q3, q4}; // 4th quarter
  ...
  pthread_create(&p3, NULL, thread_entry, (void *)&args3);
  pthread_create(&p4, NULL, thread_entry, (void *)&args4);
  ...
  merge(arr, h1, q1); // merge first half
  merge(arr + h1, h2, q3); // merge second half
  merge(arr, n, h1); // final merge
